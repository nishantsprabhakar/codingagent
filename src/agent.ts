...
function systemPrompt(root: string, projectContext: string, globalInstructions: string, projectMemoryBlock: string, skillsBlock: string): string {
  return [
    ...
    "    (with `columns`) for side-by-side comparisons, 'stats' for a handful of headline numbers as large callouts,",
    "    'timeline' (with `steps`) for a connected sequence of steps/phases, default 'title_bullets' otherwise;",
    "    `image`, `table`, and `chart` fields on any slide for figures/data (don't cram a table into bullet text as",
    "    a wall of dashes, and don't describe a trend/comparison in prose when a `chart` shows it directly — `chart`",
    "    takes {type: 'bar'|'line'|'pie'|'doughnut', categories, series: [{name, values}], title?}; use 'bar'/'line'",
    "    for comparisons or trends, 'pie'/'doughnut' with exactly one series for a part-to-whole breakdown). For",
    "    `table`, use `widths` (per-column inches) when columns naturally need different widths, and",
    "    `highlightRows` (0-indexed) to call out a 'Total' or key-finding row instead of leaving it in the normal",
    "    stripe pattern. Nested bullets via per-item `level`; a top-level `accentColor` (hex). Slide design taste, not just mechanics:",
    ...
    "  theme.renderDotList(...), theme.chartDefaults('categorical'|'circular'), theme.tableHeaderRow(cells)/",
    "  theme.tableBodyRow(cells, rowIndex, {highlight}), theme.renderStatsRow(slide, stats, x, y, w, h, 'compact'|",
    "  'large'), theme.renderTimeline(slide, steps, x, y, w, h)) provide the palette/icon-badges/shrink-to-fit-",
    "  sidebar-text/chart-and-table-styling/infographic-layouts. Call these as theme.methodName(...) — they are not",
    "  bare top-level functions, only pptxRuns and createDeckTheme themselves are. theme.chartDefaults seeds a",
    "  pptxgenjs addChart() options object with theme-consistent styling; merge in your own x/y/w/h/data and you keep",
    "  full addChart access for anything beyond it (combo charts, area/radar/scatter). Two chart gotchas worth",
    "  knowing before reaching past the preset: on a STACKED bar/column chart, dataLabelPosition must be 'ctr'/",
